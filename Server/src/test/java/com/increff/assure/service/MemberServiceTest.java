...
public class MemberServiceTest extends AbstractUnitTest {
    ...
    @Test(expected = ApiException.class)
    public void testCheckMemberTypeInvalid() throws ApiException {
        dao.insert(pojo1);
        service.checkMemberType(pojo1.getId(),MemberTypes.CUSTOMER);
    }

    @Test
    public void testCheckMemberTypeValid() throws ApiException {
        dao.insert(pojo1);
        service.checkMemberType(pojo1.getId(),MemberTypes.CLIENT);
    }
    ...
    {
        assertEquals(1,service.getByType(MemberTypes.CLIENT).size());
    }
